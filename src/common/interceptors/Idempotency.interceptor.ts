...
export class IdempotencyInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    // 1. SET THE BASE HEADER IMMEDIATELY
    // We do this here so it's locked in before any async logic or streaming starts.
    response.setHeader('X-Idempotency-Key', idempotencyKey);
    ...
    return from(this.idempotencyService.check(idempotencyKey, requestHash)).pipe(
      switchMap((result) => {
        // Case 1: Key reused with different body
        if (result.found && !result.hashMatch) {
          throw new UnprocessableEntityException({
            message: 'Idempotency key has already been used with a different request body',
            code: 'IDEMPOTENCY_KEY_REUSED',
        ...
        // Case 2: Valid cached response
        if (result.found && result.hashMatch && result.cached) {
          this.logger.log(`[IDEMPOTENT REPLAY] key=${idempotencyKey} → returning cached ${result.cached.statusCode}`);
          
          response.status(result.cached.statusCode);
          response.setHeader('X-Idempotent-Replayed', 'true'); // Only set on replays

          return of(result.cached.body);
        }

        // Case 3: Key not found, run handler
        return next.handle().pipe(
          tap({
            next: async (responseBody) => {
              const statusCode = response.statusCode;
              ...
              if (statusCode >= 200 && statusCode < 500) {
                try {
                  // WE REMOVED setHeader from here. 
                  // It was already set at the top of the function.
                  await this.idempotencyService.store(
                    idempotencyKey,
                    requestHash,
                    statusCode,
